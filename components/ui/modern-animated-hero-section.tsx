...
export const ScrambledTitle: React.FC<{ phrases?: string[] }> = ({ phrases }) => {
    ...
    if (mounted && scramblerRef.current) {
      ...
      const next = () => {
        if (scramblerRef.current) {
          scramblerRef.current.setText(activePhrases[counter]).then(() => {
            setTimeout(next, 2000)
          })
          counter = (counter + 1) % activePhrases.length
        }
      }
      next()
    }
